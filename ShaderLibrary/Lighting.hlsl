...
uint GetClusterAdditionalLightsCount()
{
    return _ClusterLightParams.x;
}

uint GetPerClusterLightsLimit()
{
    return _ClusterLightParams.y;
}
...
{
	uint i = clusterIndex1D % _ClusterDimensionParams.x;
	uint j = clusterIndex1D % (_ClusterDimensionParams.x * _ClusterDimensionParams.y) / _ClusterDimensionParams.x;
	uint k = clusterIndex1D / (_ClusterDimensionParams.x * _ClusterDimensionParams.y);

	return uint3(i, j, k);
}

float2 ComputeClusterClipSpacePosition(float2 uv)
{
	return uv * _ClusterScreenDimensionParams.xy;
}
...
#endif
	uint2 ij = screenPos / _ClusterSizeParams.xy;
	uint k = log(viewZ / _ClusterNear) * _ClusterLogGridDimY;

	return uint3(ij, k);
}

uint2 GetCullingLightIndex(float2 screenPos, float viewZ)
{
	uint clusterIndex1D = ComputeClusterIndex1D(ComputeClusterIndex3D(screenPos, viewZ));
	uint2 cullingLightIndex = _ClusterLightGridBuffer[clusterIndex1D];
	return cullingLightIndex;
}

Light GetClusterAdditionalLight(uint i, float3 positionWS)
{
